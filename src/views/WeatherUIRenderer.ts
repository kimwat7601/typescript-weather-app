...
// Data 25.03.23
///////////////////////
import {CurrentWeatherData, ForecastWeatherData} from '../utils/WeatherType';
/**
...
class WeatherUIRenderer{
     ...
     */
    renderForm(parentElem: HTMLElement): void {
     ...
     */
    renderCurrentWeather(parentElem: HTMLElement, objInfo: CurrentWeatherData): void{
        ...
        elemCurrentWeatherIcn.alt = objInfo.weatherText;
        ...
        elemCurrentTemperatureBox.textContent = String(objInfo.temperature);
        ...
        const elemCurrentWeatherOtherInfoItem = [];
        const elemCurrentWeatherOtherInfoIcn = [];
        const elemCurrentWeatherOtherInfoVal = [];
        const elemCurrentWeatherOtherinfoNum = [];
        for(let i=0; i<3; i++){
            ...
            if(i === 0){
                ...
                elemCurrentWeatherOtherinfoNum[i].textContent = String(objInfo.humidity);
                ...
                elemCurrentWeatherOtherinfoNum[i].textContent = String(Math.round(objInfo.windSpeed));
                ...
                elemCurrentWeatherOtherInfoIcnWd.style.transform = 'rotate(' + String(objInfo.windDirection) + 'deg)';
     ...
     */
    renderDayTabs(parentElem:HTMLElement, data:ForecastWeatherData[][]): void{
        const date = new Date();
        ...
        for(let i=0; i<quantity; i++){
            ...
            elemDayNum[i].textContent = String(currentDay);
     ...
     * @returns 
     */
    calcMonthendDate(currentDate: Date): number{
     ...
     * @returns 
     */
    renderForecast(objForecast: ForecastWeatherData): HTMLElement{
        ...
        elemForecastWeatherText.textContent = objForecast.weatherText;
     ...
     * @param {element} parentElem 親要素
     */
    renderForecastDays(parentElem: HTMLElement, objForecastDays: ForecastWeatherData[][]): void{
        const elemtabContWrap = document.createElement('div');
        elemtabContWrap.classList.add('tab-contents-wrap');
        const elemTabCont = [];
        const elemForecastWeatherList: HTMLElement[] = [];
        for(let i=0; i<objForecastDays.length; i++){
            ...
            objForecastDays[i].forEach((data: ForecastWeatherData) => {
     ...
     * @param {object} objForecastDays 天気予報データ
     */
    renderForecastWrap(parentElem:HTMLElement, objForecastDays: ForecastWeatherData[][]): void{
        this.removeParentChildElem(parentElem);
        this.renderDayTabs(parentElem, objForecastDays);
        this.renderForecastDays(parentElem, objForecastDays);
    }

    /**
     * ローディングの表示
     */
    renderLoading(parentElem: HTMLElement): void {
        this.removeParentChildElem(parentElem);
     ...
     * エラーメッセージの表示
     */
    renderError(parentElem: HTMLElement): void{
        this.removeParentChildElem(parentElem);
     ...
     * @param {element} parentElem 親要素
     */
    renderBody(parentElem:HTMLElement): void {
     ...
     */
    removeParentChildElem(parentElem:HTMLElement): void {
